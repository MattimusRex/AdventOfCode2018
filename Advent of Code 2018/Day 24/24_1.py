class Group:
    ...
    def attack(self, target):
        ...
        units_killed = eff_pow // target.unit_hp
        ...
        target.num_of_units = max(target.num_of_units - units_killed, 0)
    ...
    def print(self):
        print(f"ID: {self.id} # Units: {self.num_of_units}")

####TEST DATA####
# immune_system = []
# group = Group(1, 17, 5390, 2, 4507, 'fire', ['radiation', 'bludgeoning'], [], 'immune')
# immune_system.append(group)
# group = Group(2, 989, 1274, 3, 25, 'slashing', ['bludgeoning', 'slashing'], ['fire'], 'immune')
# immune_system.append(group)
...
immune_system = []
group = Group(1, 8808, 5616, 10, 5, 'bludgeoning', ['radiation'], ['cold'], 'immune')
immune_system.append(group)
group = Group(2, 900, 13511, 20, 107, 'radiation', ['radiation'], [], 'immune')
immune_system.append(group)
group = Group(3, 581, 10346, 14, 140, 'fire', ['radiation'], ['slashing'], 'immune')
immune_system.append(group)
group = Group(4, 57, 9991, 4, 1690, 'fire', ['bludgeoning'], ['slashing', 'radiation', 'fire'], 'immune')
immune_system.append(group)
group = Group(5, 4074, 6549, 2, 15, 'radiation', ['fire'], [], 'immune')
immune_system.append(group)
group = Group(6, 929, 5405, 16, 45, 'fire', [], ['bludgeoning', 'radiation'], 'immune')
immune_system.append(group)
group = Group(7, 2196, 3186, 11, 10, 'fire', ['fire'], ['radiation'], 'immune')
immune_system.append(group)
group = Group(8, 4420, 9691, 7, 21, 'fire', ['radiation'], ['fire'], 'immune')
immune_system.append(group)
group = Group(9, 3978, 2306, 12, 4, 'fire', ['cold', 'radiation'], [], 'immune')
immune_system.append(group)
group = Group(10, 1284, 4487, 19, 32, 'slashing', ['radiation', 'bludgeoning'], [], 'immune')
immune_system.append(group)
...
while len(immune_system) > 0 and len(infection) > 0:
    if len(immune_system) == 1 and len(infection) == 1:
        immune_system[0].print()
        infection[0].print()
